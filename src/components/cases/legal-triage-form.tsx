...
export default function LegalTriageForm({ caseData, onSave }: LegalTriageFormProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Triagem Jurídica (Análise de Viabilidade)</CardTitle>
          ...
          <CardContent className="space-y-6">
            <FormField control={form.control} name="triageAnalysis" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea placeholder="Descreva sua análise sobre a viabilidade, riscos e pontos de atenção do caso..." className="min-h-[120px]" {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )}/>
                ...
                <div className="mt-2 space-y-2 rounded-md border p-4">
                    {requirements.map((item) => (
                        <div key={item.id} className="flex items-center gap-2">
                            <Checkbox id={item.id} checked={item.completed} onCheckedChange={() => handleToggleRequirement(item.id)} />
                            <label htmlFor={item.id} className={cn("flex-1 text-sm", item.completed && "line-through text-muted-foreground")}>{item.text}</label>
                            <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleRemoveRequirement(item.id)}><Trash className="h-4 w-4" /></Button>
                        </div>
                    ))}
                 ...
                 <div className="flex items-center gap-2 mt-3">
                    ...
                    />
                    <Button type="button" onClick={handleAddRequirement}><Plus className="h-4 w-4" /></Button>
                </div>
            </div>
          </CardContent>
